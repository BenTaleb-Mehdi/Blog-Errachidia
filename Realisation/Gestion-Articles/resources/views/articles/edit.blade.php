...
<div class="max-w-2xl mx-auto mt-10 bg-white p-6 rounded-lg shadow">
    ...
    <form action="{{ route('articles.update', $article->id) }}" method="POST">
        ...
        <input type="text" name="title" value="{{ old('title', $article->title) }}" class="w-full p-2 border border-gray-300 rounded mb-4" required>
        ...
        <div id="hs-editor-tiptap">
            ...
            <div class="h-40 overflow-auto" data-hs-editor-field="" name="content" >{{ old('content', $article->content) }}</div>
        </div>
        </div>  
        <!-- End Tiptap -->
        <label class="block mb-2 font-medium">Statut</label>
        <select name="statut" class="w-full p-2 border border-gray-300 rounded mb-4" required>
            <option value="draft" @if($article->statut=='draft') selected @endif>Draft</option>
            <option value="published" @if($article->statut=='published') selected @endif>Published</option>
            <option value="archived" @if($article->statut=='archived') selected @endif>Archived</option>
        </select>
        ...
        <div class="space-y-2 mb-4">
            @foreach($categories as $cat)
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="categories[]" value="{{ $cat->id }}" class="w-4 h-4" @if($article->categories->contains($cat->id)) checked @endif>
                    <span>{{ $cat->name }}</span>
                </label>
            @endforeach
        </div>
